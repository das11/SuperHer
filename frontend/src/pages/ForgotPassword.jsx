...
import { ArrowLeft, ArrowRight, Lock, Mail, Loader2, KeyRound } from 'lucide-react';
...
const ForgotPassword = () => {
    ...
    const handleRequestCode = async (e) => {
        ...
        try {
            ...
            console.error(err);
            setError(err.message || "Failed to request code. Check if the email is correct.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            ...
            console.error(err);
            setError(err.message || "Failed to reset password. Check verification code and password requirements.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen w-full flex bg-[#0f0f13] text-white">
            {/* Left Side - Brand / Visuals - Reapplied from Login to keep consistency */}
            <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-gradient-to-br from-indigo-900/20 via-black to-purple-900/20 items-center justify-center p-12">
                ...
                <div className="relative z-10 max-w-lg">
                    ...
                    >
                        ...
                        Securely regain access to your campaign performance network.
                    </motion.p>
                </div>
            </div>
                ...
                >
                    <div className="mb-8">
                        ...
                        <p className="text-gray-400">
                            {stage === 1 
                                ? "Enter your email addresses and we'll send you a recovery code." 
                                : "Check your email for the verification code and set a new password."}
                        </p>
                    </div>

                    <form onSubmit={stage === 1 ? handleRequestCode : handleResetPassword} className="space-y-6">
                        
                        {/* Email Input (Always visible, readonly in stage 2) */}
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    className={`w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all ${stage === 2 ? 'opacity-60 cursor-not-allowed' : ''}`}
                        ...
                        {stage === 2 && (
                            ...
                            >
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        <KeyRound className="absolute left-3 top-3.5 h-5 w-5 text-gray-500 group-focus-within:text-purple-400 transition-colors" />
                                        <input
                                            type="text"
                                            required
                                            value={code}
                                            onChange={(e) => setCode(e.target.value)}
                                            className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all font-mono tracking-widest"
                                            placeholder="123456"
                                        />
                                    </div>
                                    <p className="text-xs text-gray-500 mt-1 ml-1">The 6-digit code sent to your email.</p>
                                </div>
                        ...
                        <AnimatePresence>
                            {success && (
                                <motion.div
                                    ...
                                    className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-sm p-3 rounded-lg flex items-start"
                                >
                                    {success}
                                </motion.div>
                            )}
                        ...
                        <motion.button
                            ...
                            type="submit"
                            disabled={isLoading}
                            className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold py-3.5 rounded-xl transition-all shadow-lg shadow-purple-900/20 flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed mt-4"
                        >
                            {isLoading ? (
                                <Loader2 className="h-5 w-5 animate-spin" />
                            ) : (
                                <>
                                    {stage === 1 ? 'Send Recovery Code' : 'Reset Password'}
